...
# Install CLI tools on Linux
install_cli_tools_linux() {
    info "Installing CLI tools on Linux"
...
# Install lazydocker (if Docker is installed)
install_lazydocker() {
    if ! command -v docker &>/dev/null; then
        skip "lazydocker (Docker not installed)"
        return 0
    fi
    ...
    if is_macos; then
        brew install lazydocker >>"$LOG_FILE" 2>&1 && \
            success "Installed lazydocker" || \
